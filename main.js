class Node {
    ...
    drawCircle(x, y, radius, fillStyle) {
      ...
      context.beginPath();
      context.arc(x * 10, y * 10, radius * 10, 0, 2 * Math.PI);
      context.fill();
    }
    ...
    drawBins() {
      // Optional: Implement this method to draw bins if needed
    }
  
    placeBuildings() {
      const binPositions = this.generateBinPositions();
      let bigBuildingPlaced = Node.isBigBuildingPlaced;
      let mediumBuildingsToPlace = 1;
      let smallBuildingsToPlace = 1;
      let housesToPlace = 10; // Adjusted to place at least 10 houses
      let treesToPlace = 1;
      let carToPlace = 1;
      ...
      binPositions.forEach((posX) => {
        ...
        while (posY < this.bottomLeft[1]) {
          ...
          } else if (!bigBuildingPlaced) {
            building = bigBuilding;
            bigBuildingPlaced = true;
            Node.isBigBuildingPlaced = true;
          ...
          let randomPosX = this.snapToGrid(
            Math.floor(
              Math.random() * (this.topRight[0] - building.width - this.x)
            ) + this.x
          );
          let randomPosY = this.snapToGrid(
            Math.floor(
              Math.random() * (this.bottomLeft[1] - building.height - this.y)
            ) + this.y
          );
          ...
          for (let i = 0; i < this.children.length; i++) {
            ...
            if (
              randomPosX < child.x + child.width &&
          ...
          if (
            !overlap &&
            randomPosY + building.height <= this.bottomLeft[1] &&
            randomPosX + building.width <= this.topRight[0]
          ) {
            ...
            randomPosX = this.snapToGrid(
              Math.floor(
                Math.random() * (this.topRight[0] - building.width - this.x)
              ) + this.x
            );
            randomPosY = this.snapToGrid(
              Math.floor(
                Math.random() * (this.bottomLeft[1] - building.height - this.y)
              ) + this.y
            );
    ...
    splitRandomly() {
      ...
      if (orientation) {
        ...
        this.children.push(
          new Node(this.x, this.y, leftWidth, this.height, this.level + 1)
        );
        this.children.push(
          new Node(partition, this.y, rightWidth, this.height, this.level + 1)
        );
      } else {
        const topHeight = partition - this.y;
        const bottomHeight = this.height - topHeight;
        this.children.push(
          new Node(this.x, this.y, this.width, topHeight, this.level + 1)
        );
        this.children.push(
          new Node(this.x, partition, this.width, bottomHeight, this.level + 1)
        );
      }
    }
    ...
    generatePartitionPoint(orientation) {
      if (orientation) {
        const partition =
          this.level === 0
            ? this.x + this.width / 2
            : this.randomBetween(this.x, this.x + this.width);
        if (
          partition - this.x < MIN_SIZE ||
          this.x + this.width - partition < MIN_SIZE
        )
          return null;
        return partition;
      } else {
        const partition =
          this.level === 0
            ...
            : this.randomBetween(this.y, this.y + this.height);
        if (
          partition - this.y < MIN_SIZE ||
          this.y + this.height - partition < MIN_SIZE
        )
          return null;
        return partition;
      }
  ...
  const bigBuilding = {
    width: 10,
    height: 5,
    images: [document.getElementById("Bangunan_besar1")],
    draw(x, y) {
  ...
  function createMap() {
    ...
    Node.isBigBuildingPlaced = false;
    Node.isGardenPlaced = false;
